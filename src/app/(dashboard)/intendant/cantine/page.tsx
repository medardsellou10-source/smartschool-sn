  ...
  return (
    <div className="space-y-6 pb-24 lg:pb-6 animate-fade-in">
      <div>
        ...
        <p className="text-sm text-ss-text-muted mt-1">Gestion des menus, abonnements et recettes</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          <div key={s.label} className="rounded-2xl p-4"
            ...
            <p className="text-xl font-black text-ss-text">{s.value}</p>
      ...
      <div className="rounded-2xl overflow-hidden" style={{ background: 'var(--ss-surface-elevated)', backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)', border: '1px solid var(--ss-glass-border)' }}>
        <div className="px-5 py-4" style={{ borderBottom: '1px solid var(--ss-glass-card-hover)' }}>
          <h2 className="text-base font-bold text-ss-text">Menu de la semaine</h2>
        </div>
        <div className="divide-y divide-white/5">
          {MENU_SEMAINE.map((m, i) => {
            const isToday = i === new Date().getDay() - 1
            return (
              <div key={m.jour} className={`flex items-center justify-between px-5 py-4 transition-colors ${isToday ? '' : 'hover:bg-ss-text/5'}`}
                style={isToday ? { background: `${ACCENT}10`, borderLeft: `3px solid ${ACCENT}` } : {}}>
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center text-lg"
                    style={{ background: isToday ? `${ACCENT}25` : 'var(--ss-glass-card-bg)' }}>
                    {isToday ? '🍽' : '📅'}
                  </div>
                  <div>
                    <p className="text-sm font-semibold" style={isToday ? { color: ACCENT } : { color: 'var(--ss-text)' }}>
                    ...
                    <p className="text-xs text-ss-text-muted">{m.plat}</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-sm font-bold text-ss-text">{m.prix} FCFA</p>
                  <p className="text-xs text-ss-text-muted">{m.abonnes} portions prévues</p>
                </div>
              </div>
            )
          })}

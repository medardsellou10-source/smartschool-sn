  ...
  return (
    <div className="space-y-6 pb-24 lg:pb-6 animate-fade-in">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          <div key={s.label} className="rounded-2xl p-4 text-center"
            ...
            <p className="text-2xl font-black text-ss-text">{s.value}</p>
      ...
      <div className="flex gap-2">
        {(['tous', 'entrant', 'sortant'] as const).map(t => (
          <button key={t} onClick={() => setOnglet(t)}
            ...
            style={onglet === t ? { background: ACCENT, color: 'white' } : { background: 'var(--ss-glass-card-bg)', color: 'var(--ss-text-muted)', border: '1px solid var(--ss-glass-border)' }}>
      ...
      <div className="space-y-3">
        {filtered.map(c => (
          <div key={c.id} className="rounded-2xl p-4 flex items-start gap-4 hover:bg-ss-text/5 transition-all"
            style={{ background: 'var(--ss-surface-elevated)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', border: c.statut === 'en_attente' ? `1px solid ${ACCENT}50` : '1px solid var(--ss-glass-border)' }}>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between gap-2">
                <p className="text-sm font-semibold text-ss-text leading-snug">{c.sujet}</p>
              ...
              <div className="flex items-center gap-3 mt-1 flex-wrap">
                ...
                <span className="text-xs text-ss-text-muted">{new Date(c.date).toLocaleDateString('fr-FR')}</span>
